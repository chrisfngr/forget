...
#SBATCH -J LLaMA-Factory-Step3                   # 作业名称
...
# Dependency: wait for step 2 to complete successfully
# Usage: sbatch --dependency=afterok:JOBID run_step3.sh
...
if [ ! -z "$DEPENDENCY_JOBID" ]; then
    ...
    echo "Note: DEPENDENCY_JOBID=$DEPENDENCY_JOBID is set. Make sure to use: sbatch --dependency=afterok:$DEPENDENCY_JOBID run_step3.sh"
fi
...
export CONFIG_FILE="${llamafactory_dir}/examples/train_full/qwen2_5vl_full_sft_step_3.yaml"
export model_path="/home/rongfeng3-c/LLM/forget/results/mllm_cl_scienceqa_odrder_2/checkpoint-2334"
# export dataset="mllm_cl_textvqa_train"
# export eval_dataset="mllm_cl_clevr_test,mllm_cl_scienceqa_test,mllm_cl_textvqa_test"   # 评测两个数据集
export output_dir="/home/rongfeng3-c/LLM/forget/results/mllm_cl_textvqa_odrder_3"
export SEED=42
export WANDB_RUN_NAME="mllm_cl_textvqa_train_$(date +%Y%m%d_%H%M%S)"  # wandb 运行名称，包含 dataset 和日期时间
